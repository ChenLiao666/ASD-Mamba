...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Helper function to load image and apply transform
        def load_image(image_path):
            image = Image.open(image_path).convert('RGB')
            image = np.array(image)  # Convert PIL image to numpy array
            if transform:  # Ensure transform is applied
                image = transform(image=image)["image"]  # Apply albumentations transform
            return image

        # Load image
        im_path = os.path.join(dataset_info['stimuli_dir'], ids.iloc[idx, 0])
        image = load_image(im_path)

        # Load saliency map
        smap_path = os.path.join(dataset_info['saliency_dir'], ids.iloc[idx, 1])
        saliency = Image.open(smap_path).convert('L')
        saliency = np.array(saliency, dtype=np.float32) / 255.
        saliency = torch.from_numpy(saliency).unsqueeze(0)

        # Load fixation map
        fmap_path = os.path.join(dataset_info['fixation_dir'], ids.iloc[idx, 2])
        fixation = Image.open(fmap_path).convert('L')
        fixation = np.array(fixation, dtype=np.float32) / 255.
        fixation = torch.from_numpy(fixation).unsqueeze(0)
...
class ValDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = np.array(image)  # Convert PIL image to numpy array
        if self.transform:
            image = self.transform(image=image)["image"]  # Apply transform (assuming it's an albumentations transform)
